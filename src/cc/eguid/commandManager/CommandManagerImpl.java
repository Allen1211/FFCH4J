...
public class CommandManagerImpl implements CommandManager {
    ...
    protected final ProgramConfig config;
     ...
     * 全部默认初始化，自动查找配置文件
     */
    CommandManagerImpl(ProgramConfig config) {
        this.config = config;
        int size = config.getSize() != null ? config.getSize() : 10;
        init(size);
    }
     ...
     * @param size
     */
    public CommandManagerImpl(ProgramConfig config, TaskDao taskDao, TaskHandler taskHandler,
